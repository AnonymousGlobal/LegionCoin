...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all LegionCoin related functionality
...
/*
    ...
    allow endless transaction that would bloat LegionCoin and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all LegionCoin specific functionality
