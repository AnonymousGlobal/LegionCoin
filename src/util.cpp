...
const char * const BITCOIN_CONF_FILENAME = "legioncoin.conf";
const char * const BITCOIN_PID_FILENAME = "legioncoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "legioncoin" is a composite category enabling all LegionCoin-related debug output
                if(ptrCategory->count(std::string("legioncoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LegionCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LegionCoin
    // Mac: ~/Library/Application Support/LegionCoin
    // Unix: ~/.legioncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LegionCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LegionCoin";
#else
    // Unix
    return pathRet / ".legioncoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty legioncoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override legioncoin.conf
