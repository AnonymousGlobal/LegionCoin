...
{
    return QCoreApplication::translate("legioncoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in legioncoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in legioncoin.qrc)
...
/** Class encapsulating LegionCoin Core startup and shutdown.
...
/** Main LegionCoin application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // legioncoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. LegionCoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(legioncoin);
    Q_INIT_RESOURCE(legioncoin_locale);
    ...
    /// 6. Determine availability of data directory and parse legioncoin.conf
    ...
    // legioncoin: links repeatedly have their payment requests routed to this process:
