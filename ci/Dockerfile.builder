...
# legioncoin_hash
RUN git clone https://github.com/legioncoinpay/legioncoin_hash
RUN cd legioncoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} legioncoin
RUN useradd -u ${USER_ID} -g legioncoin -s /bin/bash -m -d /legioncoin legioncoin
...
RUN mkdir /legioncoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /legioncoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /legioncoin-src
