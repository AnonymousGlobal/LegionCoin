...
export BUILDER_IMAGE_NAME="legioncoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/legioncoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_LGNC_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
